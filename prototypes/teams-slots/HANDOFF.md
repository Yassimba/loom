...
1. One-time **setup**: headed Playwright + 1Password service account (item `Microsoftonline` in vault `REDACTED`, TOTP included) logs into Teams, caches a Graph bearer that has `Calendars.Read*`.
...
- Yassin — `yassin.chibrani-derks@users.noreply.github.com`
- Winand — `Winand.Hulleman@users.noreply.github.com` (resolved from “Winand”)
