...
export function compilePolicies(rules: PolicyRule[]): CompiledPolicy[] {
  return rules
    ...
    .map((rule) => ({
      ...
      blockMessage: rule.blockMessage ?? DEFAULT_BLOCK_MESSAGES[rule.protection],
    }));
}
...
export function createPolicyRules(policies: CompiledPolicy[], cwd: string): Rule<PolicyMeta>[] {
  return policies.map((policy) => ({
    ...
    async check(action: Action) {
      ...
      if (policy.onlyIfExists && !action.unresolved && !(await fileExists(path, cwd))) {
        return { kind: "pass" };
      }
