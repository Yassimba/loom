...
function singleRule(cwd: string, policy: Parameters<typeof compilePolicies>[0][number]) {
...
describe("normalizeTarget", () => {
  it("prefers cwd-relative paths for targets inside cwd", () => {
    const cwd = "/repo";
    expect(normalizeTarget("/repo/config/locked.json", cwd)).toBe("config/locked.json");
  });
});
...
describe("createPolicyRules", () => {
  ...
  it("matches protected files and returns policy metadata", async () => {
    ...
    await expect(rule.check({ kind: "file", path: join(cwd, ".env") })).resolves.toMatchObject({
      kind: "match",
  ...
  it("passes allowed patterns", async () => {
    ...
    await expect(rule.check({ kind: "file", path: join(cwd, ".env.example") })).resolves.toEqual({
      kind: "pass",
    });
  });
  ...
  it("passes missing files when onlyIfExists is true", async () => {
    ...
    await expect(rule.check({ kind: "file", path: join(cwd, ".env") })).resolves.toEqual({
      kind: "pass",
    });
  });
