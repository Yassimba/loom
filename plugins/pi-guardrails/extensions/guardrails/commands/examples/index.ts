...
import { getSettingsTheme, type Scope, type SettingsTheme, Wizard } from "@aliou/pi-utils-settings";
...
type ExamplesResult = { applied: false } | { applied: true; state: ExamplesState };
...
const PRESET_LIST_HEIGHT = Math.floor(
  (EXAMPLES_CONTENT_HEIGHT - PRESET_LIST_HEADER_LINES - PRESET_DESCRIPTION_LINES) / 2,
);
...
class ExamplesWelcomeStep implements Component {
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.up) || data === "k") {
      this.selectedIndex = (this.selectedIndex - 1 + this.scopes.length) % this.scopes.length;
...
class PolicyPresetsStep implements Component {
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.up) || data === "k") {
      this.selectedIndex =
        (this.selectedIndex - 1 + POLICY_EXAMPLES.length) % POLICY_EXAMPLES.length;
  ...
  private renderMultiSelect(title: string, items: PresetListItem[], width: number): string[] {
...
class CommandPresetsStep implements Component {
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.up) || data === "k") {
      this.selectedIndex =
        (this.selectedIndex - 1 + COMMAND_EXAMPLES.length) % COMMAND_EXAMPLES.length;
...
class ExamplesReviewStep implements Component {
  ...
  render(width: number): string[] {
    const selectedPolicies = [...this.state.policyIndexes]
      ...
      .filter((item): item is (typeof POLICY_EXAMPLES)[number] => Boolean(item));
    const selectedCommands = [...this.state.commandIndexes]
      .sort((a, b) => a - b)
      .map((index) => COMMAND_EXAMPLES[index])
      .filter((item): item is (typeof COMMAND_EXAMPLES)[number] => Boolean(item));
    ...
    const lines = [
      ...
      ...selectedPolicies.flatMap((example) =>
        wrapText(`- ${example.label}: ${example.rule.protection}`, width, "    "),
      ),
      this.theme.hint(`  Dangerous commands: ${selectedCommands.length}`),
      ...selectedCommands.flatMap((example) =>
        wrapText(`- ${example.label}: ${example.pattern.pattern}`, width, "    "),
      ),
    ];

    if (!this.state.scope || selectedPolicies.length + selectedCommands.length === 0) {
      lines.push(
        "",
        this.theme.hint("  Select a scope and at least one preset before submitting."),
      );
    }
...
function getExampleScopes(): ExampleScope[] {
  ...
  return (["local", "global"] as ExampleScope[]).filter((scope) => enabled.has(scope));
}

async function applyExample(result: Extract<ExamplesResult, { applied: true }>) {
  ...
  pi.registerCommand("guardrails:examples", {
    ...
    handler: async (_args, ctx) => {
      ...
      const result = await ctx.ui.custom<ExamplesResult>(
        (_tui, theme: Theme, _keybindings, done) => createExamplesWizard(theme, scopes, done),
        { overlay: true },
      );
      ...
      ctx.ui.notify(`[Guardrails] examples applied to ${result.state.scope}.`, "info");
    },
  });
}
