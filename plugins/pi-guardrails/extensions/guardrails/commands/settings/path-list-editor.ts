...
function isAllowedPath(value: unknown): value is AllowedPath {
  ...
  return (entry.kind === "file" || entry.kind === "directory") && typeof entry.path === "string";
}
...
export class PathListEditor implements Component {
  ...
  render(width: number): string[] {
    const lines = [this.options.theme.label(` ${this.options.label}`, true), ""];
    if (this.mode === "add" || this.mode === "edit") {
      lines.push(
        this.options.theme.hint(this.mode === "edit" ? "  Edit path:" : "  New path:"),
        this.options.theme.hint(`  kind: ${kindLabel(this.draftKind)} (Tab to toggle file/dir)`),
        "",
    ...
    if (this.items.length === 0) {
      ...
      if (startIndex > 0 || endIndex < this.items.length) {
        lines.push(this.options.theme.hint(`  (${this.selectedIndex + 1}/${this.items.length})`));
      }
    }

    lines.push("");
    lines.push(this.options.theme.hint("  a: add · e/Enter: edit · d: delete · Esc: back"));
    return lines;
  }
