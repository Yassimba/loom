import { type Component, Key, matchesKey, type SettingsListTheme } from "@earendil-works/pi-tui";
...
export class ScopePickerSubmenu implements Component {
  ...
  handleInput(data: string): void {
    if (matchesKey(data, Key.up) || data === "k") {
      this.selectedIndex =
        this.selectedIndex === 0 ? this.scopes.length - 1 : this.selectedIndex - 1;
      return;
    }

    if (matchesKey(data, Key.down) || data === "j") {
      this.selectedIndex =
        this.selectedIndex === this.scopes.length - 1 ? 0 : this.selectedIndex + 1;
      return;
    }
