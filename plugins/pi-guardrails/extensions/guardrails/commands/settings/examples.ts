import type { DangerousPattern, GuardrailsConfig, PolicyRule } from "../../../../src/shared/config";
...
export const POLICY_EXAMPLES: Array<{
...
}> = [
  {
    ...
    rule: {
      ...
      allowedPatterns: [{ pattern: ".env.example" }, { pattern: "*.sample.env" }],
      protection: "noAccess",
      onlyIfExists: true,
      enabled: true,
    },
  ...
  {
    label: "SSH keys",
    description: "Blocks common SSH private key formats while allowing public key files.",
    rule: {
      id: "example-ssh-keys",
      name: "SSH keys",
      description: "Block SSH private key files",
      patterns: [{ pattern: "*.pem" }, { pattern: "*_rsa" }, { pattern: "*_ed25519" }],
  ...
  },
  {
    ...
    description: "Makes SQLite and database files read-only to avoid accidental data changes.",
    rule: {
      id: "example-database-files",
      name: "Database files",
      description: "Protect database files from modification",
      patterns: [{ pattern: "*.db" }, { pattern: "*.sqlite" }, { pattern: "*.sqlite3" }],
      protection: "readOnly",
      onlyIfExists: true,
      enabled: true,
    },
  ...
  },
  {
    ...
    rule: {
      ...
      patterns: [{ pattern: "*.crt" }, { pattern: "*.key" }, { pattern: "*.p12" }],
...
export const COMMAND_EXAMPLES: Array<{
  ...
  {
    label: "dbt run",
    description: "Prompts before running dbt models that may transform warehouse data.",
    pattern: {
      pattern: "dbt run",
  ...
  {
    label: "aws iam",
    description: "Prompts before AWS IAM commands that may change identities or permissions.",
    pattern: {
      pattern: "aws iam",
  ...
  },
  {
    ...
    description: "Prompts before importing existing infrastructure into Terraform state.",
    pattern: {
  ...
  {
    label: "gcloud iam",
    description: "Prompts before Google Cloud IAM commands that may change permissions.",
    pattern: {
      pattern: "gcloud iam",
...
export function appendPolicyRule(
  ...
  const baseId = toKebabCase(example.id || example.name || "example") || "example";
  let id = baseId;
  let i = 2;
