...
export function setPolicyRules(config: GuardrailsConfig, rules: PolicyRule[]): GuardrailsConfig {
  const updated = structuredClone(config);
  updated.policies = {
    ...(updated.policies ?? {}),
    rules,
...
export function deletePolicyRule(config: GuardrailsConfig, index: number): GuardrailsConfig {
  const rules = getPolicyRules(config);
