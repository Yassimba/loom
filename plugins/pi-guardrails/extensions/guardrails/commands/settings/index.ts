...
const FEATURE_UI: Record<GuardrailsFeatureId, { label: string; description: string }> = {
  policies: {
    label: "Policies",
  ...
  permissionGate: {
    ...
    description: "Prompt for confirmation on dangerous commands (rm -rf, sudo, etc.)",
  },
  pathAccess: {
    label: "Path access",
    ...
    },
    {
      ...
      submenu: (done) => {
        ...
        return new PatternEditor({
          ...
          onDone: () => done(`${getRule()?.allowedPatterns?.length ?? 0} items`),
        });
      },
    },
    {
...
function createSettingsConfigStore(): ConfigStore<GuardrailsConfig, ResolvedConfig> {
  return {
    ...
    getEnabledScopes: () => {
      ...
      return (["memory", "local", "global"] as Scope[]).filter((scope) => enabled.has(scope));
    },
  };
}
...
export function registerGuardrailsSettings(pi: ExtensionAPI): void {
  registerSettingsCommand<GuardrailsConfig, ResolvedConfig>(pi, {
    ...
    buildSections: (
      ...
      function updateRule(index: number, updater: (rule: PolicyRule) => PolicyRule): void {
      ...
      function patternSubmenu(id: string, label: string, context?: "file" | "command") {
        return (_val: string, submenuDone: (v?: string) => void) => {
          const items = (getNestedValue(scopedConfig, id) as DangerousPattern[] | undefined) ?? [];
      ...
      function patternConfigSubmenu(id: string, label: string, context?: "file" | "command") {
        return (_val: string, submenuDone: (v?: string) => void) => {
          const currentItems =
      ...
      const featureItems: SettingItem[] = (Object.keys(FEATURE_UI) as GuardrailsFeatureId[])
      ...
      const openPolicyEditor = (index: number, submenuDone: (v?: string) => void): Component =>
          ...
          items: [
            {
              ...
              currentValue:
                scopedConfig.pathAccess?.mode ?? `inherited: ${resolved.pathAccess.mode}`,
            ...
            },
            {
              ...
              submenu: pathListSubmenu("pathAccess.allowedPaths", "Allowed Paths"),
            },
          ],
        },
        {
          ...
          items: [
            {
              ...
              description: "Show confirmation dialog for dangerous commands (if off, just warns)",
              currentValue:
            ...
            },
            {
              ...
              submenu: patternSubmenu("permissionGate.patterns", "Dangerous Patterns", "command"),
            },
            {
            ...
            },
            {
              ...
              description: "Patterns that block commands immediately without dialog",
