...
export class PatternEditor implements Component {
  ...
  private renderListMode(width: number): string[] {
    ...
    if (this.items.length === 0) {
      ...
      for (let i = startIndex; i < endIndex; i++) {
        ...
        const text = this.theme.value(truncateToWidth(display, maxItemWidth, ""), isSelected);
        lines.push(prefix + text);
      }

      if (startIndex > 0 || endIndex < this.items.length) {
        lines.push(this.theme.hint(`  (${this.selectedIndex + 1}/${this.items.length})`));
      }
    }

    lines.push("");
    lines.push(this.theme.hint("  a: add · e/Enter: edit · d: delete · Esc: back"));
  ...
  private renderFormMode(width: number): string[] {
    ...
    // Pattern field
    const patternHint = this.context === "file" ? "(glob or regex)" : "(substring or regex)";
    const patternLabel = `  Pattern ${patternHint}:`;
    lines.push(
      patternActive ? this.theme.label(patternLabel, true) : this.theme.hint(patternLabel),
    );
    ...
    lines.push(descActive ? this.theme.label(descLabel, true) : this.theme.hint(descLabel));
    ...
    lines.push(regexActive ? this.theme.label(regexDisplay, true) : this.theme.hint(regexDisplay));
    lines.push("");
  ...
  private handleFormInput(data: string) {
    if (matchesKey(data, Key.tab) || matchesKey(data, Key.shift("tab"))) {
      ...
      this.activeField = fields[(idx + dir + fields.length) % fields.length] as Field;
      return;
    }
    ...
    const activeInput = this.activeField === "pattern" ? this.patternInput : this.descriptionInput;
