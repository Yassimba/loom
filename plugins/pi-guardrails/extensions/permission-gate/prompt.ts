...
function buildNumberedWrappedLines(
  ...
  for (const [index, logicalLine] of logicalLines.entries()) {
    ...
    const prefix = theme.fg("dim", String(lineNumber).padStart(lineNumberWidth));
...
function getCommandViewportState(
  ...
  const scrollableRows = numberedRows.filter((row) => row.logicalLineNumber !== 1);
  const scrollWindowLines = Math.max(0, COMMAND_VIEWPORT_LINES - pinnedRows.length);
...
export function createPermissionGateConfirmComponent(command: string, description: string) {
  return (
    ...
    container.addChild(
      new Text(theme.fg("warning", `This command contains ${description}:`), 1, 0),
    );
    container.addChild(new Spacer(1));
    ...
    return {
      render: (width: number) => {
        ...
        const { maxScrollOffset, pinnedRows, scrollWindowLines, scrollableRows } =
        ...
        commandBottomBorder.setText(
          buildRightAlignedBorder(width, dimBorder, linesBelow > 0 ? `↓ ${linesBelow} more` : ""),
        );
      ...
      handleInput: (data: string) => {
        ...
        const { maxScrollOffset } = getCommandViewportState(command, contentWidth, theme);
