...
describe("createPermissionGateRule", () => {
  ...
  it("matches configured dangerous command patterns", async () => {
    const rule = createPermissionGateRule({
      patterns: [{ pattern: "terraform destroy", description: "Destroy infra" }],
      useBuiltinMatchers: false,
    });
  ...
  it("can use builtin dangerous command matchers", async () => {
    ...
    expect(rule.check({ kind: "command", command: "rm -rf dist" })).toMatchObject({
      kind: "match",
    });
  });
});
...
describe("matchesAnyCommandPattern", () => {
  it("matches substring and regex command patterns", () => {
    expect(matchesAnyCommandPattern("npm publish --dry-run", [{ pattern: "npm publish" }])).toBe(
      true,
    );
    expect(
      matchesAnyCommandPattern("DROP TABLE users", [{ pattern: "^DROP TABLE", regex: true }]),
    ).toBe(true);
    expect(matchesAnyCommandPattern("npm test", [{ pattern: "npm publish" }])).toBe(false);
  });
});
...
describe("matchCommandPattern", () => {
  it("returns the matched PatternConfig", () => {
    ...
    expect(matchCommandPattern("npm publish --dry-run", patterns)).toBe(patterns[0]);
  });
  ...
  it("returns null when no pattern matches", () => {
    expect(matchCommandPattern("npm test", [{ pattern: "npm publish" }])).toBeNull();
  });
...
describe("formatAutoDenyReason", () => {
  ...
  it("falls back when description is empty string", () => {
    expect(formatAutoDenyReason({ pattern: "python -m venv", description: "" })).toBe(
      "Command matched auto-deny pattern and was blocked automatically.",
    );
  });
