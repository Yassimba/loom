...
export function isCommandAllowed(command: string): boolean {
  ...
  return compileCommandPatterns(config.permissionGate.allowedPatterns).some((pattern) =>
...
export async function saveCommandSessionGrant(command: string): Promise<void> {
  ...
  await configLoader.save("memory", {
    permissionGate: {
      allowedPatterns: [...resolved.permissionGate.allowedPatterns, { pattern: command }],
    },
  });
}
