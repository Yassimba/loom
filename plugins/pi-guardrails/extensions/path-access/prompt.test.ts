...
function renderPrompt(command?: string, width = 100, canPersistProject = true): string[] {
...
function createPrompt(width = 100, command = "cat /outside/secret.txt") {
  ...
  const prompt = component({ terminal: { columns: width }, requestRender }, theme, undefined, done);
...
describe("createPathAccessPromptComponent", () => {
  ...
  it("truncates long commands to fit the prompt width", () => {
    const lines = renderPrompt("cat /outside/very/long/path/that/keeps/going/target.txt", 48);
  ...
  it("shows the command expand shortcut only when truncated", () => {
    expect(renderPrompt("cat /outside/secret.txt", 100).join("\n")).not.toContain("expand command");

    expect(
      renderPrompt("cat /outside/very/long/path/that/keeps/going/target.txt", 48).join("\n"),
  ...
  it("renders related options side by side when wide enough", () => {
    ...
    const fileRow = lines.find((line) => line.includes("Allow file this session"));
    const dirRow = lines.find((line) => line.includes("Add directory to session workspace"));

    expect(fileRow).toContain("Allow file for this project");
    expect(dirRow).toContain("Add directory to project workspace");
  });
  ...
  it("renders one option per row when narrow", () => {
    ...
    expect(fileRow).not.toContain("Allow file for this project");
  });

  it("hides project grants when the project is untrusted", () => {
    const prompt = renderPrompt("cat /outside/secret.txt", 100, false).join("\n");

    expect(prompt).not.toContain("for this project");
