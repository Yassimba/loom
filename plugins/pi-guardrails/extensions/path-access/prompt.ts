...
function collapsedCommandRow(command: string, contentWidth: number): CommandRow {
  ...
  return {
    text,
    truncated:
      visibleWidth(command) > MAX_COMMAND_DISPLAY_WIDTH || visibleWidth(command) > commandWidth,
  };
}
...
const FILE_OPTIONS: PromptOption[] = [
  ...
  { label: "Allow file for this project", result: "allow-file-always" },
  { label: "Add directory to session workspace", result: "allow-dir-session" },
  { label: "Add directory to project workspace", result: "allow-dir-always" },
...
function optionColumnWidths(options: PromptOption[], rows: number[][]): number[] {
  const widths: number[] = [];
  for (const row of rows) {
    for (let column = 0; column < row.length; column++) {
      const option = options[row[column]];
      if (!option) continue;
      widths[column] = Math.max(widths[column] ?? 0, naturalOptionWidth(option));
    }
  }
  return widths;
}
...
function layoutWidth(layout: OptionLayout): number {
  const columnsWidth = layout.columnWidths.reduce((sum, width) => sum + width, 0);
  const gapsWidth = Math.max(0, layout.columnWidths.length - 1) * OPTION_COLUMN_GAP;
  return visibleWidth(OPTION_INDENT) + columnsWidth + gapsWidth;
}

function createOptionLayout(options: PromptOption[], width: number): OptionLayout {
  ...
  if (compact.rows.some((row) => row.length > 1) && layoutWidth(compact) <= width) {
    return compact;
  }
  ...
  return {
    ...
    columnWidths: [
      Math.min(availableWidth, Math.max(...options.map((option) => naturalOptionWidth(option)))),
    ],
  };
}
...
export function createPathAccessPromptComponent(
  ...
  return (
    ...
    const options = (showFileOptions ? FILE_OPTIONS : DIR_OPTIONS).filter(
      (option) => canPersistProject || !option.result.endsWith("-always"),
    );
    let selectedIndex = 0;
    ...
    container.addChild(new Text(theme.fg("warning", theme.bold("Outside Workspace Access")), 1, 0));
    container.addChild(new Spacer(1));
    container.addChild(
      new Text(
        theme.fg("text", `\`${toolName}\` targets a path outside the working directory.`),
        1,
        0,
      ),
    );
    const commandLine = normalizedCommand ? new Text(theme.fg("dim", ""), 1, 0) : undefined;
    if (commandLine) {
      container.addChild(commandLine);
    }
    container.addChild(new Spacer(1));
    container.addChild(new Text(theme.fg("dim", `  Cwd:  ${cwdDisplay}`), 1, 0));
    container.addChild(new Text(theme.fg("dim", `  Path: ${displayPath}`), 1, 0));
    container.addChild(new Text(theme.fg("dim", `  Dir:  ${displayDir}`), 1, 0));
    container.addChild(new Spacer(1));
    ...
    return {
      render: (width: number) => {
        ...
        if (commandLine) {
          const collapsed = collapsedCommandRow(normalizedCommand, textContentWidth);
          commandCanExpand = collapsed.truncated;
          commandLine.setText(
            theme.fg(
              "dim",
              commandExpanded ? expandedCommandRow(normalizedCommand) : collapsed.text,
            ),
          );
        }
        const commandToggleHint = commandCanExpand
          ...
          : "";
        footerLine.setText(
          theme.fg("dim", `←/→/↑/↓/Tab select · Enter select · Esc deny${commandToggleHint}`),
        );
