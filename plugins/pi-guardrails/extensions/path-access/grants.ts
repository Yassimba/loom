...
import { type AllowedPath, resolveFromCwd, toStorageGrant } from "../../src/core/paths";
...
export function resolveAllowedPaths(allowedPaths: AllowedPath[], cwd: string): AllowedPath[] {
...
export async function persistGrant(grant: PendingPathGrant): Promise<void> {
  const raw = (configLoader.getRawConfig(grant.scope) ?? {}) as Record<string, unknown>;
  ...
  const alreadyPresent = existing.some(
    (entry) => entry.kind === grant.storageGrant.kind && entry.path === grant.storageGrant.path,
  );
