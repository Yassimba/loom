...
describe("herdr extension", () => {
  ...
  it("maps matching Guardrails prompt events to Herdr blocked state", () => {
    ...
    expect(pi).toHaveEmitted(HERDR_BLOCKED_EVENT, expect.objectContaining({ active: false }));
    expect(
      pi.events.emit.mock.calls.filter(([event]) => event === HERDR_BLOCKED_EVENT),
    ).toHaveLength(2);
  });
  ...
  it("ignores missing, duplicate, and unmatched IDs", () => {
    ...
    pi.events.emit(GUARDRAILS_PROMPT_OPENED_EVENT, opened);
    pi.events.emit(GUARDRAILS_PROMPT_OPENED_EVENT, opened);
    pi.events.emit(GUARDRAILS_PROMPT_CLOSED_EVENT, createPromptClosedPayload(other));
    pi.events.emit(GUARDRAILS_PROMPT_CLOSED_EVENT, createPromptClosedPayload(opened));
    pi.events.emit(GUARDRAILS_PROMPT_CLOSED_EVENT, createPromptClosedPayload(opened));

    expect(pi).toHaveEmitted(HERDR_BLOCKED_EVENT, expect.objectContaining({ active: true }));
    expect(pi).toHaveEmitted(HERDR_BLOCKED_EVENT, expect.objectContaining({ active: false }));
    expect(
      pi.events.emit.mock.calls.filter(([event]) => event === HERDR_BLOCKED_EVENT),
    ).toHaveLength(2);
  ...
  it("balances active prompts and unsubscribes on shutdown", () => {
    pi.events.emit(GUARDRAILS_PROMPT_OPENED_EVENT, createPromptOpenedPayload(promptEvent));
    pi.events.emit(GUARDRAILS_PROMPT_OPENED_EVENT, createPromptOpenedPayload(promptEvent));

    const shutdown = registeredExtensionHandler(pi, "session_shutdown");
    assert(typeof shutdown === "function", "session_shutdown handler should be registered");
    shutdown();
