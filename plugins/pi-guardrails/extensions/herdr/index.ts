...
export default function herdr(pi: ExtensionAPI): void {
  ...
  const stopListeningForOpened = pi.events.on(GUARDRAILS_PROMPT_OPENED_EVENT, (data) => {
  ...
  const stopListeningForClosed = pi.events.on(GUARDRAILS_PROMPT_CLOSED_EVENT, (data) => {
