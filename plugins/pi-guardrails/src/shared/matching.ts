...
export function compileFilePattern(config: PatternConfig): CompiledPattern {
  ...
  return {
    test: (input) => {
      ...
      const candidate = matchFullPath ? normalized : (normalized.split("/").pop() ?? normalized);
 ...
 */
export function compileFilePatterns(configs: PatternConfig[]): CompiledPattern[] {
 ...
 */
export function compileCommandPatterns(configs: PatternConfig[]): CompiledPattern[] {
