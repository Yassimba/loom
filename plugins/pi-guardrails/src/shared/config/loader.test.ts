...
describe("guardrails config persistence", () => {
  ...
  it("adds the current config version when saving a new partial local config", async () => {
    ...
    expect(saved.pathAccess.allowedPaths).toEqual([{ kind: "directory", path: "/tmp/outside" }]);
  ...
  it("merges workspace roots by path across config scopes", async () => {
    vi.stubEnv("PI_CODING_AGENT_DIR", "/tmp/pi-agent-config-roots");

    const cwd = process.cwd();
    vol.fromJSON({
      "/tmp/pi-agent-config-roots/extensions/guardrails.json": JSON.stringify({
        version: pkg.version,
        pathAccess: {
          workspaceRoots: [{ path: "/shared", orientation: "Global orientation" }],
        },
      }),
      [join(cwd, ".pi/extensions/guardrails.json")]: JSON.stringify({
        version: pkg.version,
        pathAccess: { workspaceRoots: [{ path: "/project" }] },
      }),
    });
    ...
    await configLoader.save("memory", {
      pathAccess: { workspaceRoots: [{ path: "/shared" }, { path: "/session" }] },
    });

    expect(configLoader.getConfig().pathAccess.workspaceRoots).toEqual([
      { path: "/shared", orientation: "Global orientation" },
      { path: "/project", orientation: undefined },
      { path: "/session", orientation: undefined },
    ]);
  });
