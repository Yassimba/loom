...
describe("010-allowed-paths-objects", () => {
  describe("shouldRun", () => {
    it("runs when any entry is a legacy string", () => {
      expect(shouldRun(withAllowedPaths(["/tmp/foo", { kind: "file", path: "/a" }]))).toBe(true);
    });
  ...
  describe("run", () => {
    it("converts trailing-slash strings to directory grants", () => {
      ...
      expect(result.pathAccess?.allowedPaths).toEqual([{ kind: "directory", path: "/tmp/logs" }]);
    });
    ...
    it("drops entries with empty or invalid shapes", () => {
      const result = run(withAllowedPaths(["   ", { kind: "file" }, { kind: "x", path: "/y" }]));
      expect(result.pathAccess?.allowedPaths).toEqual([]);
    });
