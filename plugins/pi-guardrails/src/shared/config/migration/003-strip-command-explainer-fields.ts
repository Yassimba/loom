...
const REMOVED_PERMISSION_GATE_KEYS = ["explainCommands", "explainModel", "explainTimeout"] as const;

export function shouldRun(config: GuardrailsConfig): boolean {
  const raw = config as Record<string, unknown>;
  const permissionGate = raw.permissionGate as Record<string, unknown> | undefined;
...
export function run(config: GuardrailsConfig): GuardrailsConfig {
  ...
  const permissionGate = cleaned.permissionGate as Record<string, unknown> | undefined;
  if (permissionGate) {
    for (const key of REMOVED_PERMISSION_GATE_KEYS) {
