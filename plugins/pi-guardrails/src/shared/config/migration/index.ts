...
interface MigrationModule<TConfig> {
  ...
  run(config: TConfig, filePath: string, ctx: MigrationContext): Promise<TConfig> | TConfig;
}
...
function composeMigrations(
  ...
  const run: Migration<GuardrailsConfig>["run"] = async (config, filePath, ctx) => {
    let current = config;
  ...
  const message: MigrationMessageFactory<GuardrailsConfig> = (before, after, filePath, ctx) => {
    const parts: string[] = [];
    for (const sub of subs) {
      if (!sub.mod.shouldRun(before) || !sub.message) continue;
      const part =
        typeof sub.message === "function" ? sub.message(before, after, filePath, ctx) : sub.message;
...
export const migrations: Migration<GuardrailsConfig>[] = [
  composeMigrations("v0.12.0-migrations", "0.12.0", [
    ...
    { mod: envFilesToPolicies, message: envFilesToPolicies.message },
    {
      ...
      message: "pathAccess.allowedPaths was migrated from pattern objects to path strings.",
    },
  ]),
  {
    ...
    message: "Config migrated: boolean settings stored as strings were converted to true/false.",
  },
  composeMigrations("v0.14.0-path-access-migrations", "0.14.0", [
    {
      mod: allowDevNull,
      message: "pathAccess.allowedPaths was migrated to allow /dev/null by default.",
    },
    {
      mod: allowedPathsObjects,
      message: "pathAccess.allowedPaths was migrated from path strings to { kind, path } objects.",
    },
  ]),
  {
