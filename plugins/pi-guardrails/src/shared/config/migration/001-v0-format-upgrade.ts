...
import type { DangerousPattern, GuardrailsConfig, PatternConfig } from "../types";
...
export async function run(config: GuardrailsConfig, filePath: string): Promise<GuardrailsConfig> {
...
function migrateV0(config: GuardrailsConfig): GuardrailsConfig {
  ...
  if (migrated.envFiles) {
    if (migrated.envFiles.protectedPatterns) {
      migrated.envFiles.protectedPatterns = migrateStringArray(migrated.envFiles.protectedPatterns);
    }
    if (migrated.envFiles.allowedPatterns) {
      migrated.envFiles.allowedPatterns = migrateStringArray(migrated.envFiles.allowedPatterns);
    }
  ...
  if (migrated.permissionGate) {
    if (migrated.permissionGate.patterns) {
      migrated.permissionGate.patterns = migrateDangerousPatterns(migrated.permissionGate.patterns);
    }
...
function migrateStringArray(items: (string | PatternConfig)[]): PatternConfig[] {
