...
export function shouldRun(config: GuardrailsConfig): boolean {
  ...
  const requiresConfirmation = config.permissionGate?.requireConfirmation as unknown;
...
export function run(config: GuardrailsConfig): GuardrailsConfig {
  ...
  const permissionGate = migrated.permissionGate as Record<string, unknown> | undefined;
  if (
    permissionGate &&
    (permissionGate.requireConfirmation === "on" || permissionGate.requireConfirmation === "off")
  ) {
    permissionGate.requireConfirmation = permissionGate.requireConfirmation === "on";
  }
