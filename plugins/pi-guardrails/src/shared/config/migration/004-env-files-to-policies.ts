...
export function run(config: GuardrailsConfig): GuardrailsConfig {
  ...
  if (envFiles) {
    ...
    if (envFiles.allowedPatterns) rule.allowedPatterns = envFiles.allowedPatterns;
    ...
    if (Array.isArray(envFiles.protectedDirectories)) {
      const dirs = envFiles.protectedDirectories as Array<Record<string, unknown>>;
      ...
      for (const dir of dirs) {
        ...
        const normalized = dirPattern.endsWith("/**") ? dirPattern : `${dirPattern}/**`;
