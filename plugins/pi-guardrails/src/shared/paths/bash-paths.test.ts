...
describe("extractBashPathCandidates", () => {
  ...
  it("surfaces the directory go -C changes into", async () => {
    ...
    const result = await extractBashPathCandidates("go -C /tmp test ./...", CWD);
  ...
  describe("when a command has regular expression arguments", () => {
    it("keeps sed expressions in-workspace and extracts file operands", async () => {
      ...
      const result = await extractBashPathCandidates("sed 's/abc/{2,3}/g' ./file", CWD);
      expect(result).toEqual(["/work/project/s/abc/{2,3}/g", "/work/project/file"]);
    });

    it("ignores grep patterns and extracts file operands", async () => {
      const result = await extractBashPathCandidates("grep '/api/v1' ./src", CWD);
    ...
    it("keeps jq filters in-workspace and extracts file operands", async () => {
      ...
      expect(result).toEqual(['/work/project/.path | test("^/tmp/")', "/work/project/data.json"]);
    });
    ...
    it("recursively extracts paths from nested shell -c programs", async () => {
      const result = await extractBashPathCandidates("sh -c 'cat /etc/passwd > /tmp/x'", CWD);
      expect(result).toEqual(["/etc/passwd", "/tmp/x"]);
    });

    it("extracts paths from ash -c programs", async () => {
      const result = await extractBashPathCandidates("ash -c 'cat /etc/passwd'", CWD);
      expect(result).toEqual(["/etc/passwd"]);
    });
  ...
  describe("when command has interpreter arguments", () => {
    it("ignores an API endpoint passed to a Node script", async () => {
      ...
      expect(await extractBashPathCandidates(command, CWD)).toEqual([`${CWD}/scripts/call_api.js`]);
      expect(
    ...
    it("ignores an API path passed to a Python script", async () => {
      expect(await extractBashPathCandidates("python3 ./client.py /v1/users/me", CWD)).toEqual([
        `${CWD}/client.py`,
      ]);
    });
    ...
    it("still surfaces an existing path passed as script argv", async () => {
      expect(
        await extractBashPathCandidates("node ./scripts/read_file.js /etc/passwd", CWD),
      ).toEqual([`${CWD}/scripts/read_file.js`, "/etc/passwd"]);
    });

    it("treats missing script argv like other command arguments", async () => {
      expect(await extractBashPathCandidates("python3 ./deploy.py /exfil/new/data", CWD)).toEqual([
        `${CWD}/deploy.py`,
      ]);
    });
  });
  ...
  it("does not extract awk regex patterns as paths", async () => {
    const result = await extractBashPathCandidates("awk '/aaa/{flag=1} flag{print}' test.txt", CWD);
  ...
  it("ignores find expression tokens and xargs grep patterns", async () => {
    ...
    expect(result.filter((path) => !path.startsWith(`${CWD}/`))).toEqual(["/dev/null"]);
  ...
  describe("when command has path arguments", () => {
    it("extracts a single absolute path", async () => {
      expect(await extractBashPathCandidates("cat /etc/hosts", CWD)).toEqual(["/etc/hosts"]);
    });

    it("extracts multiple absolute paths", async () => {
      expect(await extractBashPathCandidates("cp /a /b", CWD)).toEqual(["/a", "/b"]);
    });

    it.each([
      ["cat ./secrets.txt /etc/passwd", [`${CWD}/secrets.txt`, "/etc/passwd"]],
      ["awk '/root/ { print }' ./secrets.txt /etc/passwd", [`${CWD}/secrets.txt`, "/etc/passwd"]],
      ["grep root ./secrets.txt /etc/passwd", [`${CWD}/secrets.txt`, "/etc/passwd"]],
      ["diff -u ./report.txt /var/log/system.log", [`${CWD}/report.txt`, "/var/log/system.log"]],
      ["cp ./secrets.txt /tmp/secrets-copy.txt", [`${CWD}/secrets.txt`, "/tmp/secrets-copy.txt"]],
      [
    ...
    it("expands ~ to home", async () => {
      expect(await extractBashPathCandidates("cat ~/file", CWD)).toEqual([`${HOME}/file`]);
    });

    it("detects Windows-style paths", async () => {
      // Quoted backslashes survive bash escape processing.
      const quoted = await extractBashPathCandidates('type "C:\\foo\\bar"', CWD);
  ...
  describe("when command has flags and redirects", () => {
    it("ignores flag arguments", async () => {
      expect(await extractBashPathCandidates("ls -la /tmp", CWD)).toEqual(["/tmp"]);
    });

    it("extracts redirect targets", async () => {
      expect(await extractBashPathCandidates("echo foo > /tmp/out", CWD)).toEqual(["/tmp/out"]);
    });

    it("extracts paths from multiple commands and redirects", async () => {
      expect(
        await extractBashPathCandidates("cat ./input && grep needle /tmp/log > ./out", CWD),
  ...
  describe("when command uses quoting", () => {
    it("handles quoted paths with spaces", async () => {
      expect(await extractBashPathCandidates('cat "/tmp/hello world"', CWD)).toEqual([
        "/tmp/hello world",
      ]);
    });
  });
  ...
  describe("when command is malformed", () => {
    it("falls back to regex tokenization on parse failure", async () => {
      ...
      const result = await extractBashPathCandidates("cat /tmp/foo 'unterminated", CWD);
