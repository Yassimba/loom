...
import { expandHomePath, isWithinBoundary, maybePathLike } from "../../core/paths/path";
import {
  commandCreatesPaths,
  hasNonPathShape,
  hasShellExpansion,
...
async function expandCandidate(candidate: string, cwd: string): Promise<string[]> {
 ...
 */
export async function extractBashTargets(
  command: string,
  cwd: string,
  options: ExtractOptions = {},
): Promise<BashPathTarget[]> {
  const { depth = 0, pathExists = existsSync } = options;
  const results = new Map<string, BashPathTarget>();

  const addResult = (path: string, checkPathAccess: boolean): void => {
    results.set(path, {
      path,
      checkPathAccess: checkPathAccess || (results.get(path)?.checkPathAccess ?? false),
    });
  };
  ...
  const addCandidate = async (
    ...
    const pathLike = forcePath || maybePathLike(token);
    ...
    for (const file of expanded) {
      ...
      const plausibleForAccess =
        pathLike &&
        (skipImplausible || isWithinBoundary(abs, cwd) || !isImplausibleLocalPath(abs, pathExists));
      addResult(abs, plausibleForAccess);
    }
  };
  ...
  try {
      ...
      if (commandName) {
        for (const arg of classifyCommandArgs(commandName, words.slice(1))) {
          if (arg.recurseShell) {
            ...
            pending.push(
              extractBashTargets(arg.token, cwd, {
                depth: depth + 1,
                pathExists,
              }).then((nested) => {
                for (const target of nested) addResult(target.path, target.checkPathAccess);
              }),
            );
          } else {
            pending.push(
              addCandidate(arg.token, arg.forcePath, commandName, words.slice(1), arg.programText),
            );
          }
        }
      } else {
    ...
    for (const match of command.matchAll(tokenRegex)) {
      ...
      if (token && !token.startsWith("-") && maybePathLike(token) && !hasNonPathShape(token)) {
        const expanded = await expandCandidate(token, cwd);
        for (const file of expanded) {
          const abs = resolve(cwd, expandHomePath(file));
          addResult(abs, true);
...
export async function extractBashPathCandidates(
  ...
  return (await extractBashTargets(command, cwd, options))
    .filter((target) => target.checkPathAccess)
