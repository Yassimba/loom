...
describe("outside-workspace plausibility filter", () => {
  describe("identifier CLIs (issue #49, PRs #51/#85)", () => {
    ...
    it("ignores library ids inside a nested shell program", async () => {
      expect(await extract(`bash -c 'npx ctx7 docs /websites/apisix "routes"'`)).toEqual([]);
    });

    it("still extracts real paths passed to an unknown runner", async () => {
      expect(await extract("npx some-cli /etc/passwd")).toEqual(["/etc/passwd"]);
    });
  });
  ...
  describe("other identifier-shaped arguments", () => {
    it("ignores owner/repo slugs", async () => {
      expect(await extract("gh pr view 12 --repo /aliou/pi-guardrails")).toEqual([]);
    });
    ...
    it("ignores git ssh remotes", async () => {
      expect(await extract("git clone git@github.com:aliou/pi-guardrails.git")).toEqual([]);
    });
  });
  ...
  describe("unchanged behaviour for known regressions", () => {
    it("still surfaces the interpreter wrapper bypass (issue #76)", async () => {
      expect(
        await extract(`powershell -Command "Get-Content -Path '/etc/passwd' -TotalCount 1"`),
      ).toEqual(["/etc/passwd"]);
    });
  ...
  describe("in-workspace noise is left alone", () => {
    ...
    it.each([
      ...
      ["gh pr view 12 --repo aliou/pi-guardrails", ["/work/project/aliou/pi-guardrails"]],
  ...
  describe("real outside-workspace access is still surfaced", () => {
    ...
    it("surfaces reads hidden in an interpreter program", async () => {
      expect(await extract(`python3 -c 'open("/etc/passwd").read()'`)).toEqual(["/etc/passwd"]);
    });
  ...
  describe("existence suppression cannot be used to escape", () => {
    it("never suppresses a redirect target", async () => {
      ...
      expect(await extract("cat secrets.txt > /exfil/dump.txt")).toEqual(["/exfil/dump.txt"]);
    });
    ...
    it("never suppresses an install -D destination", async () => {
      expect(await extract("install -D secrets.txt /exfil/data/secrets.txt")).toContain(
        "/exfil/data/secrets.txt",
      );
    });
    ...
    it("never suppresses curl --create-dirs destinations", async () => {
      expect(await extract("curl --create-dirs -o /exfil/sub/f https://example.com/f")).toEqual([
        "/exfil/sub/f",
      ]);
    });
    ...
    it("still suppresses identifiers behind the same wrappers", async () => {
      ...
      expect(await extract("sudo npx ctx7@latest docs /websites/apisix")).toEqual([]);
    });
    ...
    it("never suppresses a path-creating command nested in a shell", async () => {
      expect(await extract(`bash -c 'mkdir -p /exfil/data'`)).toContain("/exfil/data");
    });
    ...
    it("surfaces a write once the parent exists", async () => {
      ...
      expect(await extract("cp secrets.txt /tmp/exfil.txt")).toEqual(["/tmp/exfil.txt"]);
    });
  });
});
