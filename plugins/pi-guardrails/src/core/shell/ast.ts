...
import type { Command, Program, SimpleCommand, Statement, Word, WordPart } from "@aliou/sh";
...
function walkCommand(cmd: Command, callback: (cmd: SimpleCommand) => boolean | undefined): boolean {
  switch (cmd.type) {
    case "SimpleCommand":
    ...
    case "Logical":
      return walkStatement(cmd.left, callback) || walkStatement(cmd.right, callback);
    ...
    case "WhileClause":
      return (
        ("cond" in cmd && cmd.cond ? walkStatements(cmd.cond, callback) : false) ||
