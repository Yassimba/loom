 ...
 */
export function classifyCommandArgs(command: string, args: string[]): ClassifiedArg[] {
  ...
  if (cmd === "cut") return skipOptionValues(args, new Set(["-d", "--delimiter"]));
  if (cmd === "sort") return skipOptionValues(args, new Set(["-t", "--field-separator"]));
  if (cmd === "tr") return [];
...
function classifyXargsArgs(args: string[]): ClassifiedArg[] {
  const optionsWithValues = new Set(["-I", "-J", "-L", "-n", "-P", "-s", "-S", "-E"]);
  for (let i = 0; i < args.length; i++) {
    const arg = args[i] as string;
...
const SHELL_INTERPRETERS = new Set(["bash", "sh", "zsh", "dash", "ksh", "mksh", "ash"]);
...
function classifyInterpreterArgs(cmd: string, args: string[]): ClassifiedArg[] {
  const { codeFlags, fileFlags, skipFlags, shellFamily, caseInsensitive } = interpreterFlags(cmd);
  const out: ClassifiedArg[] = [];
  for (let i = 0; i < args.length; i++) {
    const arg = args[i] as string;
    ...
    if (fileFlags.has(flag)) {
      if (args[i + 1]) out.push({ token: args[++i] as string, forcePath: true });
      continue;
    }
...
function skipOptionValues(args: string[], optionsWithValues: Set<string>): ClassifiedArg[] {
  const out: ClassifiedArg[] = [];
  for (let i = 0; i < args.length; i++) {
    const arg = args[i] as string;
    if (optionsWithValues.has(arg)) {
