...
describe("classifyCommandArgs", () => {
  it("keeps unknown command arguments unchanged", () => {
    expect(tokens("cat", ["/etc/hosts", "./file"])).toEqual(["/etc/hosts", "./file"]);
  });

  it("normalizes command basenames", () => {
    expect(tokens("/usr/bin/find", ["./src", "-name", "*.ts"])).toEqual(["./src"]);
  });
  ...
  it("keeps multiple find roots before the expression", () => {
    expect(tokens("find", ["./src", "./tests", "-name", "*.ts", "-o", "-name", "*.tsx"])).toEqual([
      "./src",
      "./tests",
    ]);
  });
  ...
  it("classifies xargs-wrapped commands as their inner command", () => {
    expect(classifyCommandArgs("xargs", ["bash", "-c", "cat /etc/passwd"])).toEqual([
      { token: "cat /etc/passwd", recurseShell: true },
    ]);
  });

  it("skips xargs option values before the wrapped command", () => {
    expect(classifyCommandArgs("xargs", ["-I", "{}", "sh", "-c", "cat /etc/passwd"])).toEqual([
      { token: "cat /etc/passwd", recurseShell: true },
    ]);
  });

  it("keeps xargs-wrapped file operands", () => {
    expect(tokens("xargs", ["grep", "pattern", "./src"])).toEqual(["pattern", "./src"]);
  });
  ...
  describe("find expressions", () => {
    it("keeps find roots and ignores expression patterns", () => {
      expect(tokens("find", ["./src", "-regex", ".*/test/.*"])).toEqual(["./src"]);
    });
  });

  describe("interpreters", () => {
    it("extracts paths from interpreter inline code", () => {
      expect(tokens("python3", ["-c", 'open("/etc/passwd")'])).toEqual(["/etc/passwd"]);
    });
    ...
    it("handles PowerShell flag casing and aliases", () => {
      expect(tokens("powershell", ["-c", "Get-Content /etc/passwd"])).toEqual(["/etc/passwd"]);
      expect(tokens("pwsh", ["-COMMAND", "Get-Content /etc/passwd"])).toEqual(["/etc/passwd"]);
    });
    ...
    it("marks only inline code paths as program text", () => {
      ...
      expect(classifyCommandArgs("python3", ["-c", 'open("/etc/passwd")'])).toEqual([
        { token: "/etc/passwd", programText: true },
      ]);
    });
  });
