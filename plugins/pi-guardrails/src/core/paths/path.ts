 ...
 */
export type AllowedPath = { kind: "file"; path: string } | { kind: "directory"; path: string };
...
export async function canonicalizeFromCwd(input: string, cwd: string): Promise<string> {
  const absolute = resolveFromCwd(input, cwd);
  ...
  async function canonicalize(path: string): Promise<string> {
    try {
      ...
      return parent === path ? path : join(await canonicalize(parent), basename(path));
    }
  }
 ...
 */
export function isWithinBoundary(targetAbsPath: string, rootAbsPath: string): boolean {
...
export function normalizeForDisplay(absPath: string, cwd: string): string {
  ...
  if (absPath === home || absPath.startsWith(`${home}/`) || absPath.startsWith(`${home}\\`)) {
 ...
 */
export function toStorageGrant(absPath: string, isDirectory: boolean): AllowedPath {
  return {
