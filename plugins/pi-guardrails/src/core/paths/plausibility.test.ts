...
describe("hasShellExpansion", () => {
  ...
  it.each(["/etc/passwd", "~/.ssh/id_rsa", "./src/index.ts", "/tmp/a-b", "a$"])(
    "leaves %s alone",
    (token) => {
...
describe("createsPaths", () => {
  it.each(["mkdir", "install", "rsync", "tar", "git", "/usr/bin/mkdir", "MKDIR"])(
    "treats %s as path-creating",
    (command) => {
      expect(createsPaths(command)).toBe(true);
    },
  );

  it.each(["cat", "ctx7", "go", "grep", undefined])(
    "does not treat %s as path-creating",
    (command) => {
...
describe("isImplausibleLocalPath", () => {
  ...
  it("accepts a path that exists", () => {
    expect(isImplausibleLocalPath("/etc/passwd", exists(["/etc/passwd"]))).toBe(false);
  });

  it("accepts a missing file whose parent exists", () => {
    expect(isImplausibleLocalPath("/tmp/new.txt", exists(["/tmp"]))).toBe(false);
  });

  it("rejects a missing path whose parent is also missing", () => {
    expect(isImplausibleLocalPath("/websites/apisix", exists(["/"]))).toBe(true);
  });
