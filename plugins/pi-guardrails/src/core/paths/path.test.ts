...
describe("canonicalizeFromCwd", () => {
  it("resolves symlinks through the nearest existing parent", async () => {
    await mkdir("/outside", { recursive: true });
    await mkdir("/tmp", { recursive: true });
    await symlink("/outside", "/tmp/link");

    await expect(canonicalizeFromCwd("/tmp/link/new.txt", "/workspace")).resolves.toBe(
      "/outside/new.txt",
    );
  });
});
