...
describe("isPathAllowed", () => {
  ...
  describe("when entry is an exact file grant", () => {
    it.each([
      ...
      expect(isPathAllowed(path, [{ kind: "file", path: "/foo/bar" }])).toBe(expected);
    });
  });
  ...
  describe("when entry is a directory grant", () => {
    it.each([
      ...
      expect(isPathAllowed(path, [{ kind: "directory", path: "/foo/bar" }])).toBe(expected);
    });
  });
  ...
  describe("when allowedPaths has multiple entries", () => {
    ...
    ])("with mixed file + directory grants, returns $expected for $path", ({ path, expected }) => {
      expect(
        isPathAllowed(path, [
...
describe("checkPathAccess", () => {
  ...
  describe("when mode is allow", () => {
    it("always returns allow, even for paths outside cwd and without UI", () => {
      ...
      expect(checkPathAccess("/etc/hosts", "/etc/hosts", state).kind).toBe("allow");
      expect(checkPathAccess("/work/project/src", "src", state).kind).toBe("allow");
    });
  });

  describe("when path is inside cwd", () => {
    it.each([{ mode: "block" as const }, { mode: "ask" as const }])(
      "returns allow in $mode mode",
      ({ mode }) => {
        const state = base({ mode });
        expect(checkPathAccess("/work/project/src/file", "src/file", state).kind).toBe("allow");
      },
    );

    it("returns allow when path equals cwd", () => {
      expect(checkPathAccess(cwd, ".", base({ mode: "ask" })).kind).toBe("allow");
    });
  });
