...
export function hasNonPathShape(token: string): boolean {
  return URL_SCHEME.test(token) || REMOTE_TARGET.test(token) || WILDCARD_SEGMENT.test(token);
}
...
export function effectiveCommandName(
  ...
  for (let hops = 0; hops < 3; hops++) {
    ...
    if (TRANSPARENT_PREFIXES.has(cmd)) {
      ...
      const index = rest.findIndex((arg) => SUBCOMMAND_RUNNERS[cmd]?.includes(arg));
      if (index === -1) return cmd;
      next = rest[index + 1];
...
export function commandCreatesPaths(commandName: string | undefined, args: string[] = []): boolean {
