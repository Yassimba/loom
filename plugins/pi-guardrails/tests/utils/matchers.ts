...
const toHaveEmitted: Matcher<MatcherState, [channel: string, expectedPayload: unknown]> = function (
  received,
  channel,
  expectedPayload,
) {
  ...
  const pass = emitted.some(([, payload]) => this.equals(payload, expectedPayload));
