...
- For symbols, references, diagnostics, and structural refactors, use Serena before broad grep/read exploration when available.
...
export const EXPLORATION_REMINDER =
  "Pause native exploration. Unless the user requested native-only work, query Serena for code structure or Codebase Memory for architecture and impact before another read or grep. Then verify the result with native tools.";

function isCodeIntelligenceCall(toolName: string, input: unknown): boolean {
  if (toolName === "mcp__serena" || toolName === "mcp__codebase_memory_mcp") return true;
  if (toolName !== "mcp" && toolName !== "mcpScript") return false;
  const call = JSON.stringify(input)?.toLowerCase() ?? "";
  return (
    call.includes("serena") || call.includes("codebase-memory") || call.includes("codebase_memory")
  );
}
