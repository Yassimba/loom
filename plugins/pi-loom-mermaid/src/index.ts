...
export default function piLovelyMermaid(pi: ExtensionAPI): void {
  ...
  pi.on("before_agent_start", (event) => ({
    systemPrompt: `${event.systemPrompt}\n\nYou can communicate visually using fenced \`mermaid\` blocks, rendered directly in the user’s session. Supported diagram types: flowchart, sequence, state, class, ER, mindmap, timeline, pie, and git graph. Always use Mermaid diagrams when they make the message easier to understand than prose alone. Keep them compact, with short labels; prefer top-down layouts for narrow terminals. Always visualize node diffs when it makes sense: use \`:::red\` for removed, \`:::green\` for added, and \`:::orange\` for changed nodes. When custom colors are needed, prefer colored box outlines and avoid colored backgrounds unless the user asks for them.`,
  }));
}
