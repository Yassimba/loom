---
name: browser-usage
description: Core agent-browser usage guide — read before running any agent-browser command. Use when the user asks to interact with, test, or automate a website — clicking, filling forms, extracting data, taking screenshots, logging in, staying logged in.
---
...
Never automate a login form when a human can do it once. A `--restore` key
persists cookies + storage by name and reloads them on every later run:

```bash
# Human, once per site (visible window, log in, close it):
agent-browser --headed --session github --restore open https://github.com/login
agent-browser --session github close          # state saved

# Agent, every run after — headless, already logged in:
agent-browser --session github --restore --restore-check-text "Sign out" \
  open https://github.com/notifications
```

With a `--restore-check-*` guard the status line reports `restore: loaded_but_invalid`
when the saved login expired. If a
snapshot shows a login page, stop and ask the human to redo the first step —
