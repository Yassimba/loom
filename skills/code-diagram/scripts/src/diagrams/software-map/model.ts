import {
  ...
  type DataStoreForeignKeyRef,
} from "../../canvas/data-store-schema";
import {
  CANVAS_DATA_STORE_KINDS,
  type CanvasDataStoreKind,
} from "../../canvas/model";
import type { CanvasRelationshipSemanticKind } from "../../canvas/semantics";
import type { ChangeState } from "../../document/model";

export type SoftwareRelationshipSemanticKind = CanvasRelationshipSemanticKind;
...
export type SoftwareChangeStatus = ChangeState;
...
export interface SoftwareSemanticRelationshipInput extends SoftwareRelationshipBaseInput {
  kind: "semantic";
  semanticKind?: SoftwareRelationshipSemanticKind;
...
export type SoftwareDataStoreForeignKeyRef = DataStoreForeignKeyRef;
...
type ElementDraft = NormalizedSoftwareElement;
...
export function defineSoftwareMap(
  ...
  for (const element of elements) Object.freeze(element);
...
function validateElementShape(
  ...
  if (
    ...
    !CANVAS_DATA_STORE_KINDS.includes(rawInput.kind as CanvasDataStoreKind)
  ) {
    errors.push(
...
function resolveEndpoint(
  ...
  for (const candidate of candidates) {
    ...
    if (parseDataStoreSchemaEndpoint(candidate, elementsByPath)) return candidate;
  }
  return undefined;
}
