...
import { sourceRangeSchema, type SourceEvidenceResolver } from "./diagram-family";

const targetSchema = z.strictObject({
  __kind: z.literal("db-target-ref"),
  storeId: z.string(),
  storeKind: z.enum(["relational", "document"]),
  storeLabel: z.string(),
  storeDataStoreKind: z
    .enum(["database", "objectStore", "bucket", "artifactStore", "fileStore"])
    .optional(),
  storeSoftwareMapPath: z.string().optional(),
...
export async function compileDatabaseLens(
  ...
  return compiledDatabaseLensSchema.parse({
    ...model,
      ...
      model.useCases.map(async (useCase) => ({
        ...
        operations: await Promise.all(
          useCase.operations.map(async (operation) => {
            const source = await evidence.resolveRange(operation.anchor.peek.props);
            return {
              ...operation,
              anchor: {
                ...operation.anchor,
                peek: {
                  ...operation.anchor.peek,
                  resolution: { source },
                },
              },
              source,
