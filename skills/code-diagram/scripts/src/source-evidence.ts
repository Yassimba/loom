...
import type { CodePeekProps } from "./authoring";
import type { SourceEvidenceResolver, SourceRange } from "./diagram-family";
import { patchChangedLines, type CallStackChangedLines } from "./call-stack-diff";
...
type DiffFile = { path: string; previousPath?: string; lines: CallStackChangedLines | null };
...
export class SourceEvidenceService implements SourceEvidenceResolver {
  ...
  resolveRange(input: CodePeekProps): Promise<SourceRange> {
    const key = `${this.revision ?? "none"}:${input.graph ?? "head"}:${input.file}:${input.fromLine}:${input.toLine}`;
    let pending = this.cache.get(key);
    if (!pending) {
      pending = this.readRange(input);
      this.cache.set(key, pending);
    }
    return pending;
  }

  changedLines(file: string, side: "base" | "head"): CallStackChangedLines | null {
