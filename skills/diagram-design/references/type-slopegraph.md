...
- **The two axes must never differ from each other** — not in scale, not in origin, not in transform. A shared *origin* matters as much as a shared scale: shifting one axis tilts every slope by the same amount, so the series still rank correctly against each other while every rate is wrong. That is the harder version to spot by eye, so verify slope and origin separately.
...
| Binding | Without it |
|---|---|
| `data-from` / `data-to` on the line | A missing label would otherwise drop the series silently from a value review. |
...
Together these bindings keep every visible string tied to the series, endpoint, and state it describes.

**Keep verified geometry free of `transform`.** A transform on a series line, bound label, ancestor `<g>`, or CSS rule moves the rendered mark away from its declared coordinates. Bake offsets into the coordinates. The rotated value-axis caption is fine because it is neither verified geometry nor a bound label.
...
- `assets/example-slopegraph.html` — minimal light
- `assets/example-slopegraph-dark.html` — minimal dark
- `assets/example-slopegraph-full.html` — full editorial
