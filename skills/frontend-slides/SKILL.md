...
- [diagrams.md](diagrams.md) — Read when a slide carries a diagram — architecture, flow, sequence, timeline, quadrant, funnel, Venn, org chart, Sankey: author it with `/diagram-design`, inline the exported SVG
...
1. **De-slop the copy** — Run `/stop-the-slop` over every visible string in the deck: headlines, body copy, captions, chart labels, and speaker notes. Stance follows Phase 1 Question 3 — *Preserve* when the user supplied the content, *Improve* when the copy was written from a topic or rough notes. Slide copy is the shortest text in the deck and the most read, so slop shows fastest there. Done when every visible string has been through the pass, the rewrites are back in the HTML, and any slide whose copy grew has been re-checked for overflow.
...
| [diagrams.md](diagrams.md)                                                         | Diagram slides via `/diagram-design`, inlined as SVG                 | Phase 3, diagram slides only |
