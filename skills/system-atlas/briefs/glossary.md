...
Sources: the topic records in <WORKDIR>/topics/*.json and, when needed, the code.

Cover 45 to 60 terms in these groups, in this order, with a "group" field on each: 1 the product's domain concepts; 2 the organisation's data sources and systems; 3 cloud and tooling; 4 the product itself, with one line per repository and its main run modes. The orchestrator gives you a TERMS list to start from. Add terms the topics use that a newcomer would not know.
